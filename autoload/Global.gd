...
func _ready():
	...
	# optional utility mode for exporting the in-memory test data to external/data/*
	if _maybe_export_external_data_templates():
		return
...
func _maybe_export_external_data_templates() -> bool:
	if OS.get_cmdline_user_args().has("--export-test-data"):
		FileLoader.export_test_data()
		get_tree().quit()
		return true
	return false
