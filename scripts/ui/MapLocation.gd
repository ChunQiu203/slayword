...
func init(_location_data: LocationData):
	...
	if location_data.location_obfuscated and not location_data.location_visited:
		map_label.text = I18N.tr_key("map.unknown") # unvisited obfuscated locations are marked hidden
	else:
		map_label.text = I18N.tr_enum("map", LocationData.LOCATION_TYPES.keys()[location_data.location_type])
